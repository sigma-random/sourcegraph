...
type Mutation {
    ...
    createCampaign(
        ...
        namespace: ID!
        campaignSpec: ID! # CampaignSpec.id
    ): Campaign!
    ...
    applyCampaign(
        ...
        campaignSpec: ID! # CampaignSpec.id
        # If set, return an error if the campaign identified using the namespace and spec parameters
        # does not match the campaign with this ID. This lets callers use a stable ID that refers to
        # a specific campaign during an edit session (and is not susceptible to conflicts if the
        # underlying campaign is moved to a different namespace or renamed).
        #
        # TODO - this is kinda weird but seems useful to avoid someone accidentally clobbering
        # changes on a campaign when someone else renamed/moved it without their knowledge (eg if
        # they kept a browser tab open or reused a CLI history entry).
        campaign: ID
    ): Campaign!
    ...
    createChangesetSpec(
        # TODO(sqs): the raw changeset spec, as JSON or something?
        changesetSpec: String!
    ): ChangesetSpec!

    # Upload a campaign spec that will be used in a future update to a campaign (with
    # applyCampaign).
    #
    ...
    createCampaignSpec(
        # The campaign spec as YAML (TODO also allow JSON for easier machine-creation or creation from JS web clients?).
        campaignSpec: String!
        # Changeset specs that were locally computed and then uploaded using createChangesetSpec.
        changesetSpecs: [ID!]!
    ): CampaignSpec!

    # Compute the delta (difference) between a campaign's desired state (spec) and its actual state
    # (status).
    #
    # TODO(sqs): There may be a query form of this that creates a CampaignDelta without persisting
    # it, for when this just needs to be shown in the UI immediately.
    computeCampaignDelta(
        campaign: ID!
...
# An immutable description of the desired state of a changeset in a campaign.
type ChangesetSpec implements Node {
    id: ID! # unguessable ID
    expiresAt: DateTime
}

# An immutable description of the desired state of a campaign.
...
#
# NOTE: This would be used for BOTH (1) showing applyCampaign previews AND (2) after you
# applyCampaign, showing you 'what still needs to be done to reconcile the actual state with the
# desired state' (ie the 'status indicator').
type CampaignDelta implements Node {
    # TODO(sqs): add more fields here
